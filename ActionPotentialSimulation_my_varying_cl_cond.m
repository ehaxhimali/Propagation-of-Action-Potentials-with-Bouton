...
%These simulations are all done with a 1 mew m thick axon%
...
%all the chloride conductance values we will test
g_values = [0,8.9,17.7,26.6,35.5,44.3];

V_values = zeros(total_time/d_t,J,length(g_values)) ; % this will hold all the potential values at each compartment and at each unit of time
...
m_values = zeros(total_time/d_t,J,length(g_values)) ;
h_values = zeros(total_time/d_t,J,length(g_values)) ; 
...
for g = 1:length(g_values)
    g_cl =g_values(g)*10^(-2); 

    % before beginning the loop set the values of m and h and V etc for the
    % first time step
    V_values(1,:,g) = -80 ;
    m_values(1,:,g) = (alpha_m(V_values(1,:,g) )*d_t)./(1+(alpha_m(V_values(1,:,g)) + beta_m(alpha_m(V_values(1,:,g)),m_inf(V_values(1,:,g)) ))*d_t);
    h_values(1,:,g) = (alpha_h(beta_h(V_values(1,:,g) ),h_inf(V_values(1,:,g)))*d_t)./(1+(alpha_h(beta_h(V_values(1,:,g)),h_inf(V_values(1,:,g))) + beta_h(V_values(1,:,g)))*d_t);
    ...
    for n  = 2:total_time/d_t - 1
       ...
       %while abs(V_0-V_1) > 10^(-4)
            ...
            for j = 1:J
                ...
                alpha_m_current = alpha_m(V_values(n,j,g));
                m_inf_current = m_inf(V_values(n,j,g));
                beta_m_current = beta_m(alpha_m_current,m_inf_current);
                h_inf_current = h_inf(V_values(n,j,g));
                beta_h_current = beta_h(V_values(n,j,g));
                ...
                % first solve the triagonal system of equations for U
                    ...
                    disp(['rest =' num2str(d_t/C_values(j)*(g_Na_m*real(m_0(j))^3*real(h_0(j))+g_leak_m))])       
                    %disp(['rest =' num2str(d_t/C_values(j)*(g_Na_a*m_0(j)^3*h_0(j)+g_leak_a))])
            
                    D = 1 - L - U + d_t/C_values(j)*(g_Na_m*real(m_0(j))^3*real(h_0(j))+g_leak_m); %sum through conductances            
                    ...
                    if j ~= ceil(J/2) % case when not buoton
                        if  (n == 1  ) && j == 1  % we have an injected current in the first time step in first compartment
                            ...
                            B(j) = V_0(j) + d_t/C_values(j) * (g_Na_m*real(m_0(j))^3*real(h_0(j))*E_na+g_leak_m*E_l) + d_t/C_values(j) * I_inj;
                        else
                            B(j) = V_0(j) + d_t/C_values(j) * (g_Na_m*real(m_0(j))^3*real(h_0(j))*E_na+g_leak_m*E_l) + d_t/C_values(j);            
                        end    
...
plot(time,V_values(:,ceil(J/2),1))
hold on;
plot(time,V_values(:,ceil(J/2),2))
hold on;
plot(time,V_values(:,ceil(J/2),3))
hold on;
plot(time,V_values(:,ceil(J/2),4))
hold on;
plot(time,V_values(:,ceil(J/2),5))
hold on;
plot(time,V_values(:,ceil(J/2),6))

title("Unmyelinated Axon")
xlabel("Time (msecs)")
ylabel("Membrane Potential (mV)")
legend('0.0 nS', '8.9 nS', '17.7 nS','26.6 nS','35.5 nS', '44.3 nS')
